...
describe.only('testing adding blogs', async() => {
  ...
  test.only('a valid blog can be added ', async () => {
    ...
    const onlyUser = await helper.usersInDb()
    
    const userForToken = {
      username: onlyUser[0].username,
      id: onlyUser[0].id,
    }
  ...
  test.only('blog cannot be added if there is no token', async () => {
    const newBlog = {
      title: 'jotain',
      author: 'jotain',
      url: 'www.w.fi',
